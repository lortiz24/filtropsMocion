...
export class CheckInService {
  ...
  constructor(private readonly firebaseDB: Firestore) {
    this.participationCollection = collection(
      this.firebaseDB,
      "event/colombia4.0/attendees"
    );
  }
  ...
  async saveUserParticipation({
    names,
    email,
    points,
    newParticipation = false,
  }: SaveParticipationOfUser) {
    const now = new Date();
    ...
    if (previousParticipation) {
      ...
      const newPoints =
        points === undefined ? previousParticipation.points : points;

      const newParticipationDateList: Timestamp[] = [
        ...previousParticipation.participationDateList,
      ];
      if (newParticipation) {
      ...
      await updateDoc(userExperienceRef, {
        points: newPoints,
        updateAt,
        participationDateList: newParticipationDateList,
      });

      return docId;
    } else {
      const experience = this.getExperienceById({
        experienceId: this.experienceId,
      });
      if (!experience)
        return console.error("la experiencia no esta registrado");
  ...
  async getAttendeeByEmail({
    ...
    if (!querySnapshot.empty) {
      if (querySnapshot.docs.length > 1)
        throw new Error("email asignado a dos usuarios");
  ...
  async getUsersParticipation() {
    ...
    if (!querySnapshot.empty) {
      ...
      const usersParticipation: Participation[] = querySnapshot.docs.map(
        (doc) => ({
          id: doc.id,
  ...
  listeningUsersParticipation(
    onSetUsersParticipants: (data: Participation[]) => void
  ) {
    return onSnapshot(
      this.participationCollection,
      (querySnapshot) => {
